...
# Produces static html/js/css under dist/ - copied into the final image as ./publish.
FROM node:22-bookworm-slim AS frontend-build
WORKDIR /app/frontend
COPY frontend/ ./
# Empty = browser calls /api on the same host as teh page (same domain as express).
ENV VITE_API_URL=
...
# Express serves API routes and static files from public/ (the Vite bulild from stage 1).
